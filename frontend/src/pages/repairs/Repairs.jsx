...
import { MdDevices, MdNewReleases } from "react-icons/md";
...
function Repairs({ path }) {
  ...
  const filterItems = ["All", "New", "In Progress", "Ready for collection", "Completed", "Overdue"];
  ...
  const handleGetPage = () => {
    getAllRepairs({ page: currentPage, limit: limit, userId: user?.id }, setMaintenanceDevices, setTotalPages);
  };
      ...
      <div className="flex flex-col  gap-2">
        ...
        <div className="flex flex-col gap-10">
          <div className="grid grid-cols-5 grid-rows-1 gap-3">
            ...
            <div className="flex items-center bg-white gap-3 border shadow-md rounded-md p-5">
              <div className="w-[52px] h-[52px] flex items-center justify-center bg-purple-50 border border-purple-600 rounded-full">
                <HiOutlineWrenchScrewdriver size={25} className="text-purple-600" />
              </div>
              <div className="flex flex-col gap-2">
                <span className="text-sm text-black font-semibold">All Repairs</span>
                <span className="font-bold text-xl">{allRepairsStats?.total_repairs}</span>
              </div>
            </div>
            {/* */}
            <div className="flex items-center bg-white gap-3 border shadow-md rounded-md p-5">
              <div className="w-[52px] h-[52px] flex items-center justify-center bg-red-50 border border-red-600 rounded-full">
                <MdNewReleases size={25} className="text-red-600" />
              </div>
              <div className="flex flex-col gap-2">
                <span className="text-sm text-black font-semibold">New</span>
            ...
            {/* */}
            <div className="flex items-center bg-white gap-3 border shadow-md rounded-md p-5">
              <div className="w-[52px] h-[52px] flex items-center justify-center bg-orange-50 border border-orange-600 rounded-full">
                <TbProgressCheck size={25} className="text-orange-600" />
              </div>
              <div className="flex flex-col gap-2">
                <span className="text-sm text-black font-semibold">In progress</span>
            ...
            <div className="flex items-center bg-white gap-3 border shadow-md rounded-md p-5">
              ...
              <div className="flex flex-col gap-2">
                <span className="text-sm text-black font-semibold">Overdue</span>
                <span className="font-bold text-xl">{allRepairsStats?.overdue_repairs}</span>
              </div>
            </div>
            <div className="flex items-center bg-white gap-3 border shadow-md rounded-md p-5">
              <div className="w-[52px] h-[52px] flex items-center justify-center bg-green-50 border border-green-600 rounded-full">
        ...
        <div className="flex flex-col">
          <div className="bg-white rounded-md shadow-md p-2">
            <div className="flex justify-between gap-3">
              <div className="repair-filer">
                {filterItems.map((item) => (
                  <span className={`repair-filer-item ${active === item ? "active" : ""}`} onClick={() => setActive(item)}>
                    {item}
                  </span>
                ))}
              </div>
            ...
            <DeviceMaintenanceRepairs repairs={searchResults ? searchResults : maintenanceDevices} currentPage={currentPage} setCurrentPage={setCurrentPage} totalPages={totalPages} limit={limit} />
          </div>
        </div>
      </div>
