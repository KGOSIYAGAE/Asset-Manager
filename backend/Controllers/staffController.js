...
const createStaff = async (req, res) => {
  try {
    const { name, surname, phone_number, email, staff_no, position_id, department_id, contract_type, isActive, start_date, end_date } = req.body;

    if (!name || !surname || !phone_number || !email || !staff_no || !position_id || !department_id || !contract_type || !isActive || !start_date || !end_date) {
    ...
    const VALUES = [name, surname, phone_number, email, staff_no, contract_type, isActive, position_id, department_id, start_date, end_date];
